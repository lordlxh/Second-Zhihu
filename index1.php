...
<html lang="Zh">
    ...
    <style>
        ...
        <!--====== Favicon Icon ======-->
		<link rel="stylesheet" type="text/css" href="assets/css/ourself.css">
		...
		<div class="mainbody">
			<div class="body2-1">
                ...
                <?php 
                    ...
                    if(!mysqli_select_db($connect,"question")){echo "<script>alert('链接失败');</script>";}
                  ...
                  for($i=0;$i<count($main);$i++){
                    echo '<div class="body2-1-title" >
                    <div >
                        <div style="font-weight:bold;font-size:25px">',$main[$i],'</div>';
                    echo "<div style='font-size:20px;font-weight:normal;' onclick='enter(this)' id=$qid[$i]>查看回答</div></div>";
                    echo "<span class='for' onclick='dianzan(this)' id=$qid[$i]>▲喜欢",$likes[$i],'</span>';
                    echo '<span class="for" onclick="c(this)">▼踩</span>
                    <!--这下一行的input是个单选框，做伸缩用的。不用连数据库QAQ-->
                    <label style="display:inline"><input type="checkbox"class="comment">评论
                        <img src="./assets/images/taolun.png" style="margin-left:-66px;margin-top:-5px"><br>
                        <div class="commentbody">';
                  echo '<div style="color:#6D6D6D;font-weight:bold;font-size:15px;"></div>';
                  $p=0;
                    for($j=0;$j<count($qcid)&&$p<5;$j++){
                        if($qcid[$j]==$qid[$i]){
                    echo        
                            '<div class="commentbody-1">
                                <div><img src="assets/images/logo/logo-1.png"height=20px>',$commenter[$j],'</div>  
                                <div style="font-weight:normal;">',$content[$j],'</div>
                            </div>';
                        $p++;}}
                            $p=0;
                    echo        '<div>
                                <form action="put comment.php" method="get">
                                    <input name="content" class="textinput" type="text" value="输入你的回答">
                                    <input type="hidden" name="qid" value="',$qid[$i],'">
                                    <input type="hidden" id="commenter" name="commenter" value="',$_GET["uname"],'">
