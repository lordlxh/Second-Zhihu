...
//列表存在,注册用户账号信息
	...
	if(in_array($uname,$data)){unset($data);
		echo "<script  language = 'javascript' type = 'text/javascript'>alert('用户名已存在'); window.location.href = 'http://localhost/zhuce.html';</script>";
		$sign=0;
	}
	else{//检测用户名，密码是否合法
	...
	echo "<script  language = 'javascript' type = 'text/javascript'> window.location.href = 'http://localhost/denglu.html';</script>";}
